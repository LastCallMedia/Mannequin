    ...
    select(markdownAST, 'link')
        .forEach(function(el) {
            ...
            if (!url.hostname && url.pathname) {
                ...
                const referencedPath = path.resolve(path.dirname(absPath), url.pathname.replace(/^\/+/, ''));
                ...
                // If it's not a markdown file, attempt to link it directly to the repo.
                    ...
                    let ghUrl = parse(`https://github.com/LastCallMedia/Mannequin/tree/master/${repoRelative}`)
                    ghUrl.hash = url.hash
