...
abstract class PatternTestCase extends TestCase {
  ...
  public function testGetAliases() {
    $this->assertEquals(static::PATTERN_ALIASES, $this->getPattern()->getAliases());
  }
  ...
  public function testVariableDefinition() {
    ...
    $pattern = $this->getPattern();
    $this->assertEquals(new Definition(), $pattern->getVariableDefinition());
    $this->assertEquals($pattern, $pattern->setVariableDefinition($definition));
  ...
  public function testVariableSets() {
    ...
    $pattern = $this->getPattern();
    $this->assertEquals(['default' => $defaultSet], $pattern->getVariableSets());
    $this->assertEquals($pattern, $pattern->addVariableSet('foo', $fooSet));
    $this->assertEquals(['default' => $defaultSet, 'foo' => $fooSet], $pattern->getVariableSets());
    $pattern->addVariableSet('default', $overriddenDefault);
    $this->assertEquals(['default' => $overriddenDefault, 'foo' => $fooSet], $pattern->getVariableSets());
  }
  ...
  public function testGetFile() {
    ...
    if($pattern instanceof TemplateFilePatternInterface) {
      ...
      $this->assertEquals(self::TEMPLATE_FILE, $pattern->getFile()->getPathname());
    }
  }
}