...
class NestedPatternVariableSubscriberTest extends TestCase {
  ...
  public function testDiscoversNestedPatternInVariable() {
    ...
    $pattern->getVariableDefinition()->willReturn(new Definition(['bar' => 'pattern']));
    $pattern->getVariableSets()->willReturn(['default' => new Set('Default', ['bar' => 'baz'])]);
    ...
    $this->dispatchDiscover($subscriber, $pattern->reveal(), $collection->reveal());
  }
}