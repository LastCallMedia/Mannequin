...
class YamlMetadataParserTest extends TestCase {
  ...
  public function testParsesDefinition() {
    ...
    $this->assertEquals(new Definition(['foo' => 'bar']), $parsed['definition']);
  }

  public function testParsesValueToDefaultSet() {
    $parsed = (new YamlMetadataParser())->parse('value: {foo: bar}');
    $this->assertEquals(['default' => new Set('Default', ['foo' => 'bar'])], $parsed['sets']);
  }

  public function testParsesValuesToDefaultSet() {
    $parsed = (new YamlMetadataParser())->parse('values: {foo: {bar: baz}}');
    $this->assertEquals(['foo' => new Set('foo', ['bar' => 'baz'])], $parsed['sets']);
  }

  public function testParseValueNameToDefaultSet() {
    $parsed = (new YamlMetadataParser())->parse('value: {_name: My Default, _description: Some description }');
    $this->assertEquals(['default' => new Set('My Default', [], 'Some description')], $parsed['sets']);
  }

  public function testParseValuesNameToSetName() {
    $parsed = (new YamlMetadataParser())->parse('values: {foo: {_name: My Foo, _description: Some description }}');
    $this->assertEquals(['foo' => new Set('My Foo', [], 'Some description')], $parsed['sets']);
  }

  public function getInvalidMetadataTests() {
    return [
      ['{,]', 'foo', new TemplateParsingException('Unable to parse YAML metadata in foo')],
      ['bar', 'foo', new TemplateParsingException('Metadata must be an array in foo')],
      ['name: {}', 'foo', new TemplateParsingException('name must be a string in foo')],
      ['description: {}', 'foo', new TemplateParsingException('description must be a string in foo')],
      ['tags: ""', 'foo', new TemplateParsingException('tags must be an array in foo')],
      ['variables: ""', 'foo', new TemplateParsingException('variables must be an array in foo')],
      ['value: ""', 'foo', new TemplateParsingException('value must be an array in foo')],
      ['values: ""', 'foo', new TemplateParsingException('values must be an array in foo')],
    ];
  }
   ...
   */
  public function testInvalidMetadata($input, $identifier, \Exception $expectedException) {
    try {
