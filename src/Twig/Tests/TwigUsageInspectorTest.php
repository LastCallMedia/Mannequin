...
class TwigUsageInspectorTest extends TestCase {
  ...
  public function getTestSets() {
    return [
      ...
      ["{% embed 'myembedded' %}{%block content %}test{% endblock %}{% endembed %}", ['myembedded']],
      ["{%block main%}{%embed 'myembedded'%}{%endembed%}{%endblock%}", ['myembedded']],
      ["{%extends 'myparent' %}{%block main%}{%embed 'myembedded'%}{%endembed%}{%endblock%}", ['myembedded', 'myparent']],
    ];
  }
  ...
  public function testSet($twigSrc, array $expectedUsed) {
    ...
    $used = $inspector->inspectLinked(new \Twig_Source($twigSrc, 'mytemplate', ''));
