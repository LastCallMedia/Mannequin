...
class FetchingCodeBlock extends Component {
    ...
    fetch() {
        ...
        this.props.fetch(this.props.src)
    ...
    render() {
        ...
        return (
            <div className={loading ? 'FetchingCodeBlock loading' : 'FetchingCodeBlock'}>
                {error && <Callout type="alert" title="Error fetching code" content={
                    <p>{error.message}</p>
                }/>}
                {loading && <Loading />}
