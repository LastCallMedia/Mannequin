...
function Menu({tree, settings}) {
    return (
        <ul className={`MenuList ${settings.className}`}>
            {tree.map((child, i) => <MainMenuItem key={i} leaf={child} className={settings.itemClassName} childSettings={settings.children} onNavigate={settings.onNavigate} />)}
        </ul>
    )
}
Menu.propTypes = {
    ...
    render() {
        ...
        return (
            <li className={`MenuItem ${className}`}>
                ...
                {leaf.children && <Menu tree={leaf.children} settings={childSettings} />}
            </li>
        )
    }
}