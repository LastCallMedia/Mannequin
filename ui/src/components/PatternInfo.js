    ...
    return (
        <div className={cx('PatternInfo', className)}>
            <div className="inner">
                ...
                <PatternInfoInfo className="info" pattern={pattern} used={used} variant={variant} />
...
const PatternInfoInfo = ({pattern, variant, used}) => {
    return (
        <div className="info">
            ...
            {pattern.metadata.description && <PatternInfoSection title="Description">{pattern.metadata.description}</PatternInfoSection>}
            {used.length > 0 && <PatternInfoSection title="Used">{used.map(p => <Link to={`/pattern/${p.id}`}>{p.name}</Link>)}</PatternInfoSection>}
        </div>
    );
}
        ...
        return (
            <div className="code">
                ...
                <div className="button-group">
                    {variant && <a className={cx('button', src === variant.source ? 'primary' : 'secondary')} onClick={this.switchMode} data-src={variant.source} data-language={'html'}>HTML</a>}
                    {pattern && <a className={cx('button', src === pattern.source ? 'primary' : 'secondary')} onClick={this.switchMode} data-src={pattern.source} data-language={pattern.metadata.source_format}>Raw</a>}
                </div>
            </div>
        )
    }
